...
| **Audio** | Acoustic recognition is **planned but deliberately not started** — no capture path, no microphone in the device tree, no model, no phase. The spectrogram check in `tools/dataset/` *rejects* audio artefacts from the visual set; it is not audio support. See [ROADMAP scope](ROADMAP.md#scope-audio-is-planned-and-deliberately-not-started). |
